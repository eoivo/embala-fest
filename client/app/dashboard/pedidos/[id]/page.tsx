...
import { ArrowLeft, Calendar, Clock, Download, Printer, Send, User } from "lucide-react"
import Link from "next/link"

export default function DetalhePedidoPage({ params }: { params: { id: string } }) {
  ...
  const pedido = {
    ...
    desconto: 0,
    itens: [
      { id: 1, nome: "Embalagem para Bolo G", quantidade: 2, preco: 4.5, total: 9.0 },
      { id: 2, nome: "Balões Coloridos (pct 50un)", quantidade: 1, preco: 15.0, total: 15.0 },
      { id: 3, nome: "Pratinhos Descartáveis (pct 10un)", quantidade: 3, preco: 8.0, total: 24.0 },
      { id: 4, nome: "Velas de Aniversário", quantidade: 2, preco: 5.0, total: 10.0 },
      { id: 5, nome: "Copos Descartáveis (pct 50un)", quantidade: 2, preco: 12.0, total: 24.0 },
    ],
  }
  ...
  return (
    <DashboardShell>
      <DashboardHeader heading={`Pedido ${pedido.id}`} description="Detalhes do pedido">
            ...
            <CardFooter className="flex flex-col items-end">
              <div className="w-full max-w-xs space-y-2">
                ...
                <div className="flex justify-between font-medium">
                  <span>Total:</span>
                  <span>R$ {(pedido.valorTotal - pedido.desconto).toFixed(2)}</span>
                </div>
              </div>
            </CardFooter>
          </Card>
